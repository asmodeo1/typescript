...
// TypeScript: indicamos el tipo del parámetro
function animar(casilla: HTMLElement) {
...
function comprobarVictoria(c1: HTMLElement, c2: HTMLElement, c3: HTMLElement) {
...
function jugar(evt: MouseEvent) {
    ...
    // TypeScript: debemos indicar el tipo evt.target para que no de error en textContent
    (evt.target as HTMLElement).textContent = juegan;
    jugados++;
...
function nuevaPartida() {
    ...
    const casillas = document.getElementsByClassName("casilla") as HTMLCollectionOf<HTMLElement>;
